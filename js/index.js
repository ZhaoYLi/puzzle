...
//默认按顺序排好，大DIV第九块没有，所以为0，我们用0表示空块
d[1] = 1;
d[2] = 2;
d[3] = 3;
d[4] = 4;
...
var target_d = 0;
//用来找出小DIV可以去的位置，如果返回0，表示不能移动，如果可以移动，则返回可以去的位置的编号
target_d = whereCanTo(i);
if(target_d != 0){//如果target_d不为0，则表示可以移动，切target_d就是小DIV要去的大DIV的位置编号
	d[i] = 0;//把当前的大DIV编号设置为0，因为当前小DIV已经移走了，所以当前大DIV没有装小DIV了。  
	d[target_d] = id;//   把目标大DIV设置为被点击的小DIV的编号
	//设置被点击的小DIV的位置，把他移到目标大DIV的位置
...
function whereTo(cur_div){
	//判断是否可移动函数，参数是大DIV的编号，不是小DIV的编号，因为小DIV编号跟可以去哪没关系，小DIV时会动的
	var j = 0;
	var move_flag = false;
...
function random_d(){
	for(var i=9;i>1;--i){
	    ...
	    if(d[i]!=0){
	    	document.getElementById("id" +d[i]).style.left = d_posXY[to][0] +"px";
	    	document.getElementById("id" +d[i]).style.top = d_posXY[to][1] +"px";	    	
	    }
	    //把当前的DIV位置设置为随机产生的DIV位置
	    var tem = d[to];
	    d[to] = d[i];
	    d[i] = tem;
	    //然后把他们两个的DIV保存的编号对调一下
	}
}
